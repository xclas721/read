# 客戶資訊工具 — 第一階段局部 Spec（email 查詢這條線）精進版

> **定位**：W3 第一階段作業的「可實作」版本。範圍同 [kyson_w3-spec-第一階段-email查詢.md](kyson_w3-spec-第一階段-email查詢.md)，但補上技術契約（資料模型、API 契約、遮罩／可信度演算法），目標是**AI 讀完能直接實作、不需腦補**。
> **完整版**見 [kyson_w3-spec.md](kyson_w3-spec.md)（第二階段，全工具）。
...
讓 AI Agent 收到客訴 email 時，帶**寄件人 email**呼叫 API，在秒級時間內取得**結構化 JSON**（客戶身分、服務、SLA、聯絡窗口），作為後續 Summary 與工具鏈的識別起點。第一階段僅 DB 唯讀查詢，不含後台維護、批次、Summary 產生。
...
| R1.3 | 命中時回傳該客戶訂閱服務（ACS／3DSS／veriid）及 SLA，並彙總全戶最嚴格 SLA | M      | R3           |
...
| 資料庫   | PostgreSQL 15                                 | 第一階段 DB only                      |
...
- **JWT 內含**：`sub`（呼叫主體）、`role`（`Admin` | `Viewer` | `Agent`）、`exp`。token 由內部 SSO／IdP 簽發（簽發流程屬完整 spec，本階段僅驗章 + 驗 exp + 驗 role 是否允許本 API）。
...
- 遮罩在**回傳序列化前**套用；DB 儲存層 email／name 不明碼（ISO-27001 導入後再上欄位加密，屬完整 spec）。
...
| 無 active 服務，或僅靠 domain 推定（本階段不歸戶，多為 not_found） | `low`    |
...
| ----- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| T1.1  | R1.1       | Given DB 有客戶 A、active 聯絡人 `alice@a-bank.com` / When `Agent` 角色帶有效 JWT 查 `alice@a-bank.com` / Then HTTP 200、`status=found`、含 A 的 `name`(遮罩)／`issuer_oid`／`region`／`customer_status`；**P95 ≤ 2s**（規模：~~300 家、~~3000 筆聯絡人、低併發）；**寫 audit**（caller_subject、email hash、timestamp、result=found） | `Alice@A-Bank.com` 大小寫不同須命中同筆                                        |
...
| T1.9  | R1.5, R1.8 | Given ~3000 筆聯絡人載入 / When 連續 50 次單筆查詢（低併發） / Then **P95 ≤ 2s、P99 ≤ 5s**，且每次皆寫 audit                                                                                                                                                                                                       | 壓測資料量與正式量差異須記錄                                                       |
...
- **技術**：第一階段 DB only（PostgreSQL）；部署 GCP + K8S；唯讀服務，無寫入端點；參數化查詢防注入。
...
- [ ] **JWT 簽發與 role 對應**：IdP 來源、role 與後台 RBAC 是否同一套 — 待主管／完整 spec 對齊。
...
| 本文件需求 | 完整 spec | 規格段落                    | 本文件驗收             |
| ----- | ------- | ----------------------- | ----------------- |
| R1.1  | R1      | §C、§E                   | T1.1              |
| R1.2  | R2      | §C                      | T1.2              |
| R1.3  | R3      | §E(effective_sla_level) | T1.3              |
| R1.4  | R4      | §C、§E                   | T1.4, T1.11       |
| R1.5  | R13     | §D、§E                   | T1.5, T1.9, T1.10 |
| R1.6  | R14     | §D                      | T1.6              |
| R1.7  | R19     | §B(audit_log)           | T1.7              |
| R1.8  | R18     | §F                      | T1.8              |
